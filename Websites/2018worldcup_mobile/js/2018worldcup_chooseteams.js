...
$.fn.getForecastForm = function(){
    ...
    if(target.find('.setCountry').length>0){
      ...
      if(teams=='t2'){
        target.append(dom).find('.setCountry').stop().animate({top:0,left:0},speed)
              .end().find('img').css({width:'50px','margin-left':'23px'}).stop().animate({width:'154px','margin-left':'0px'},speed);
        first.find('img').css({width:'154px','margin-left':'0px'}).stop().animate({width:'50px','margin-left':'23px'},speed);    
  ...
  function movetoNonefinish(){
    if($(".nonefinish").length>0){
      var compareDis = $('.nonefinish').eq(0).offset().top; 
      var minTopdis;
      for(var i=0; i<$('.nonefinish').length; i++){
        minTopdis = Math.min(compareDis, $('.nonefinish').eq(i).offset().top)
      }
      $('html, body').stop().animate({scrollTop:minTopdis - 50},500);      
    }          
  }
  ...
  b4Group.off('click').on('click',function(){
    ...
    if(g4==4){
      if(fcstStatus.t4  && !self.hasClass('active')){
        ...
        moveWinner(self.siblings(), block3.find('.'+name+(num/1+1)+ ''),'t3');
    ...
    if(g2==2){
      if(fcstStatus.t2){
        if(!self.hasClass('active')){
          ...
          self.siblings().removeClass('active').addClass('unchoose').find('.competition').attr('data-result',0);
          self.removeClass('unchoose').toggleClass('active').find('.competition').attr('data-result',1);
          moveWinner(self,block1.find('.'+name+num+''),'t2');
